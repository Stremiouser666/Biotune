...
class AudioEngine {
  ...
  constructor() {
    ...
    if (typeof navigator !== 'undefined' && (navigator as any).requestMIDIAccess) {
      (navigator as any).requestMIDIAccess()
        ...
        .catch((e: any) => {
          console.warn("MIDI access restricted or denied by permissions policy:", e.message);
        });
    }
    ...
    this.pianoSampler = new Tone.Sampler({
      ...
      onerror: () => { 
        if (!useCDN) {
          ...
          console.warn("All sampler loading failed. Using synthesizer fallback.");
          this.isLoaded = false;
  ...
  triggerDrum(type: 'soft' | 'hard' | 'roll', time?: any) {
    ...
    if (this.mode === 'custom' && this.customDrumPlayers) {
      ...
      if (type === 'hard' && p.has("kick") && p.player("kick").loaded) { p.player("kick").start(triggerTime); played = true; }
      else if (type === 'soft' && p.has("snare") && p.player("snare").loaded) { p.player("snare").start(triggerTime); played = true; }
      else if (type === 'roll' && p.has("hat") && p.player("hat").loaded) { p.player("hat").start(triggerTime); played = true; }
  ...
  private triggerNoteAtTime(rowIndex: number, time: any) {
    ...
    notesToPlay.forEach(note => {
      ...
      const isSampledReady = this.mode === 'sampled' && this.pianoSampler && this.pianoSampler.loaded;
      const isCustomReady = this.mode === 'custom' && this.customPianoSampler && this.customPianoSampler.loaded;
  ...
  private sendMidi(status: number, data1: number, data2: number) {
    if (this.midiOutput) {
      try {
        (this.midiOutput as any).send([status, data1, data2]);
      } catch (e) {}
    }
  }
