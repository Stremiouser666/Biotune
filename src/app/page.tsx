            ...
            <div className="w-full animate-scroll-open space-y-4">
              <Accordion type="single" collapsible value={accordionValue} onValueChange={setAccordionValue} className="w-full space-y-3">
                  ...
                  <AccordionContent className="pt-3 px-1 space-y-3">
                    <div className="p-3 bg-white/30 rounded-2xl border border-white/40 shadow-sm space-y-3">
                      <div className="flex items-center justify-between px-1">
                        ...
                        <button onClick={() => { audioEngine?.randomiseMelody(); setSessionVersion(v => v + 1); }} className="flex items-center gap-1 px-3 py-1.5 bg-primary/20 rounded-lg text-[9px] font-headline">
                      ...
                      <div className="flex items-center gap-2 px-1">
                        ...
                        <div className="flex gap-1 flex-wrap">
                          {["C","C#","D","D#","E","F","F#","G","G#","A","A#","B"].map(note => (
                            <button key={note} onClick={() => { setRootNote(note); audioEngine?.setRootNote(note); }} className={cn("px-1.5 py-0.5 text-[8px] rounded font-headline", rootNote === note ? "bg-primary text-white" : "bg-white/40")}>{note}</button>
                          ))}
                        </div>
                      </div>
                ...
                <AccordionItem value="rhythm" className={cn("border-none transition-all", onboardingStep === 2 && "ring-4 ring-primary ring-offset-4 rounded-2xl")}>
                  <AccordionTrigger className="flex gap-3 px-4 py-3 bg-white/40 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm hover:no-underline">
                    <div className="flex items-center gap-2"><Zap className="w-4 h-4 text-primary" /><span className="font-headline text-[13px] text-black">RHYTHM STUDIO</span></div>
                  </AccordionTrigger>
                  <AccordionContent className="pt-3 px-1 space-y-3">
                    <div className="p-3 bg-white/30 rounded-2xl border border-white/40 shadow-sm space-y-3">
                      <div className="flex justify-end px-1">
                        <button onClick={() => { audioEngine?.randomiseDrums(); setSessionVersion(v => v + 1); }} className="flex items-center gap-1 px-3 py-1.5 bg-primary/20 rounded-lg text-[9px] font-headline">
                          <Dice5 className="w-3 h-3" /> RANDOMISE
                        </button>
                      </div>
                ...
                <AccordionItem value="sound" className="border-none">
                  <AccordionTrigger className="flex gap-3 px-4 py-3 bg-white/40 backdrop-blur-md rounded-2xl border border-white/60 shadow-sm hover:no-underline">
                    <div className="flex items-center gap-2"><Sliders className="w-4 h-4 text-primary" /><span className="font-headline text-[13px] text-black">SOUND PALETTE</span></div>
                  </AccordionTrigger>
                          ...
                          {(['piano', 'kick', 'snare', 'hat'] as const).map(type => (
                            <div key={type} className="relative">
                              <label className="flex flex-col items-center gap-1 p-2 bg-white/40 rounded-lg cursor-pointer hover:bg-white/60 transition-all">
                                ...
                                <input type="file" accept="audio/*" className="hidden" onChange={(e) => handleFileUpload(type, e)} />
                              </label>
                            </div>
                          ))}
                        </div>
